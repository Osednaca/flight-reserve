...
export class FlightService {
  ...
  searchFlights(origin: string, destination: string, date: string): Observable<Flight[]> {
    return this.http.get<Flight[]>(`${this.apiUrl}/flights?origin=${origin}&destination=${destination}&date=${date}`);
  }

  bookFlight(flightId: number, passengerName: string): Observable<any> {
    return this.http.post(`${this.apiUrl}/bookings`, { flightId, passengerName });
  }
}